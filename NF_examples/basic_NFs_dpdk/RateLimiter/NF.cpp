...
long _t1=100;
int _t2=1;
long _t3=1000000000/8;
int _t4=0;
int _t5=0;
State<long> token(_t1);
int process(Flow &f){

if(sigprocmask(SIG_BLOCK,&newmask,NULL)==-1) std::cout<<"block failed"<<endl;
	//if(token[f]>0) printf("%ld\n", token[f]);
if ((*(int*) f.headers[Tag]) == _t2){
tick.it_value.tv_sec = 1;
tick.it_value.tv_usec =0;
setitimer(ITIMER_REAL, &tick, NULL);
token[f]=_t3;
}
else if (((*(int*) f.headers[Tag]) == _t4)&&(token[f] < (*(int*) f.headers[Iplen]))){
return -1;
}
else if (((*(int*) f.headers[Tag]) == _t5)&&(token[f] >= (*(int*) f.headers[Iplen]))){
token[f]=token[f] - (*(int*) f.headers[Iplen]);
}
...
int read_local_pcap(int argc, char* argv[]){
    ...
    sprintf(filePath,"/home/hhy/NFD-EXP/pcap/1/1_10_%s.pcap",argv[1]);
        //printf("path : %s\n", filePath[loop]);
...
int main(int argc, char* argv[])
{
    ...
    printf("time = %f,    pps = %f,     sum = %ld,    drop = %ld,    drop_rate = %f,    pass_rate = %f bps,  set_rate = %ld bps,   byte_size = %s \n",total, _s / total, _s, _drop, (double)_drop/(double)_s, (double)(_s-_drop)/total* stoi(argv[1])*8, _t3*8, argv[1]);
    FILE *fp = fopen("rate_limiter.txt","a");
    fprintf(fp, "time = %f,    pps = %f,     sum = %ld,    drop = %ld,    drop_rate = %f,    pass_rate = %f bps,  set_rate = %ld bps,   byte_size = %s \n",total, _s / total, _s, _drop, (double)_drop/(double)_s, (double)(_s-_drop)/total* stoi(argv[1])*8, _t3*8, argv[1]);
    fclose(fp);
