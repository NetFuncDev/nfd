/*
  ...
  Copyright (c) 2012, 2013 Ulf Ã…strÃ¶m
...
ser_tra_t * ser_new_tra(const char * id, const size_t size, ser_tra_t * att)
{
  ...
  tra = (ser_tra_t*)malloc(sizeof(ser_tra_t));
  dup_id = strdup(id);
			    ...
			    const char * tag, const size_t offset)
{
  ...
  field =(ser_field_t*) malloc(sizeof(ser_field_t));
...
bool ser_job_realloc_result(ser_job_t * job, const int len)
{
  ...
  new_result = (char*) realloc(job->result, len + 1);
...
/**
   ...
   #category Internal
*/
bool ser_job_cat(ser_job_t * job, const char * s)
{
  int cat_len;
  int old_len;
...
bool ser_job_realloc_holders(ser_job_t * job, const int n)
{
  ...
  new_holder = (ser_holder_t *)realloc(job->holder, sizeof(ser_holder_t) * n);
				    ...
				    const char * d_tag, const size_t d_ai)
{
  ...
  sptr = (ser_subst_ptr_t *)malloc(sizeof(ser_subst_ptr_t));
