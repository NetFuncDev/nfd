...
/******************************************************************/
extern "C" int cpu_encrypt (char* akey,char* plain,unsigned long length,char* cipher)
{  
  ...
  // calculate the Fpadding
  padding = 16 - (numbytes + 4) % AES_BLOCK_SIZE;
//  printf("Padding file with %d bytes\n", padding);

  buf = (uint8_t*)calloc(numbytes + 4 + padding, sizeof(uint8_t));
  ...
  cipher = (char*)calloc(numbytes, sizeof(char));
  for(unsigned long i = 0; i < numbytes;i++)
    cipher[i] = buf[i];
