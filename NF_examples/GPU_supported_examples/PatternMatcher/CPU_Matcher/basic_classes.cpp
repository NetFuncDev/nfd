...
void Flow::clean() {	
    ...
    u_char * payload = this->pkt + ethernet_header_length + ip_header_length;
    u_char * content = (u_char *) this->field_value["payload"];
    if (payload != content)
        memcpy(payload, content, strlen((char *)content));
